...
allforest = read.csv("https://github.com/indiaaugusta/Tsimane-Cytokines/blob/master/forest_plots_Th.csv", stringsAsFactors = FALSE)

order = c( 'LPS', 'H1N1')
allforest$Type = factor(allforest$Type, levels=order)

order1 = c( 'Male', 'Female')
allforest$Sex = factor(allforest$Sex, levels=order1)

dotCOLS = c("#f9b282", "#a6d8f0")
barCOLS = c("#de6b35", "#008fd5")

ggplot(allforest, aes(x=Type, y=Estimate, ymin=Lower_CI, ymax=Upper_CI, color = Sex, fill = Sex))+
...
# create forest plot df. cytokine, beta, beta+1.96*se, beta-1.96*se
suppforest = read.csv("https://github.com/indiaaugusta/Tsimane-Cytokines/blob/master/forest_plots_proanti.csv", stringsAsFactors = FALSE)
suppforest = suppforest[!(is.na(suppforest$Type)),]

order = c( 'LPS', 'H1N1')
suppforest$Type = factor(suppforest$Type, levels=order)

order1 = c( 'Male', 'Female')
suppforest$Sex = factor(suppforest$Sex, levels=order1)

dotCOLS = c("#f9b282", "#a6d8f0")
barCOLS = c("#de6b35", "#008fd5")

supptable = ggplot(suppforest, aes(x=Type, y=Estimate, ymin=Lower_CI, ymax=Upper_CI, color = Sex, fill = Sex))+
 geom_linerange(size=.8,position=position_dodge(width = 0.5)) +
geom_hline(yintercept=0, lty=2) +
geom_point(size=3, shape=21, colour="white", stroke = 0.5,position=position_dodge(width = 0.5)) +
scale_fill_manual(values=barCOLS, guide=TRUE)+
...
met = read.csv("https://github.com/indiaaugusta/Tsimane-Cytokines/blob/master/meta_stat.csv", stringsAsFactors = FALSE)
...
```{r}
serum = read.csv("https://github.com/indiaaugusta/Tsimane-Cytokines/blob/master/serum_forest.csv", stringsAsFactors = FALSE)
